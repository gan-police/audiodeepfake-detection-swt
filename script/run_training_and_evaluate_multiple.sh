...
# expects first argument ($1) to be a directory containing multiple config.yaml files
...
# directory to collect all the finished configs
mkdir $1/done

for file in $1/*; do
  if [[ $file == *.yaml ]] ; then
    echo "work on configuration '$file'"
    ...
    python -O -u script/config_train.py --config $file
    # gather training results
    sleep 5
    python -O -u script/gather_training_results.py --config $file
    echo "finished training"

    # start evaluating models
    srun --ntasks=$SLURM_NTASKS --cpu-bind=verbose --cpus-per-task=$SLURM_CPUS_PER_TASK --threads-per-core=$SLURM_THREADS_PER_CORE --mem="${MEM}G" --gres="gpu:${GPUS}" --gpu-bind=closest --exact --exclusive --kill-on-bad-exit=0 --wait=0 \
    python -O -u script/config_evaluate.py --config $file ${@:2}
    # gather evaluation results
    sleep 5
    python -O -u script/gather_evaluation_results.py --config $file
